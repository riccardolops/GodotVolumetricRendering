...
namespace VolumetricRendering
{
    ...
    public class RawDatasetImporter
    {
        ...
        public VolumeDataset Import()
        {
            ...
            {
                GD.PrintErr("The file does not exist: " + filePath);
                return null;
            }
            ...
            {
                GD.PrintErr($"The dimension({dimX}, {dimY}, {dimZ}) exceeds the file size. Expected file size is {expectedFileSize} bytes, while the actual file size is {fs.Length} bytes");
                reader.Close();
                fs.Close();
                return null;
            }
        ...
        private void ImportInternal(VolumeDataset dataset, BinaryReader reader, FileStream fs)
        {
            ...
            dataset.rotation = Quaternion.FromEuler(new Vector3(Mathf.DegToRad(90.0f), 0.0f, 0.0f));
        }
